...
class FeriadoController extends Controller{
    ...
    public function index(){  
        ...
        $dados["clientes"]          = FeriadoService::listaclientes("cliente");
        $dados["instrutores"]       = FeriadoService::listainstrutor();
        $dados["formatos"]          = Service::lista("posvenda_treinamento_formato");
        $dados["tipoocupacao"]      = Service::lista("posvenda_treinamento_ocupacao");
        $dados["horarios"]          = FeriadoService::listaHorarioagenda();
        $dados["horariossala"]      = FeriadoService::listaHorario("posvenda_agenda_horario");
        $dados["produtos"]          = FeriadoService::listaiproduto("produto");
        $dados["usos"]              = Service::lista("posvenda_treinamento_uso");
        $dados["salas"]             = Service::lista("posvenda_treinamento_sala");
        $dados["view"]              = "Feriado/Index";
    ...
    public function salvar(){
        ...
        if(FeriadoService::salvar($agenda, $this->campo, $this->tabela)){
            $this->redirect(URL_BASE."agenda"."?id_responsavel=".$agenda->id_responsavel."&data_inicio1=".$di."&data_fim1=".$df."&pesquisar=".$pe);
        }else{
            FeriadoService::salvar($agenda, $agenda->id_agenda, $this->tabela);
            if(!$agenda->id_agenda){
    ...
    public function exportar(){
        $dados["lista"] = FeriadoService::lista();
    ...
    public function filtro()
    {
        ...
        $dados["lista"]             = FeriadoService::lista();
        $dados["datas"]             = FeriadoService::listapordata($data1,$data2);
        $dados["clientes"]          = FeriadoService::listaclientes("cliente");
        $dados["instrutores"]       = FeriadoService::listainstrutor();
        $dados["formatos"]          = Service::lista("posvenda_treinamento_formato");
        $dados["tipoocupacao"]      = Service::lista("posvenda_treinamento_ocupacao");
