...
abstract class Model{
    ...
    //Retorna uma lista da tabela ordem Decrescente
    function allordemdes($conn, $tabela =null){
        $tabela = ($tabela) ? $tabela: $this->tabela;
        try {
            $sql = "SELECT * FROM ". $tabela ." ORDER BY id_item DESC";
            $stmt = $conn->query($sql);
            return $stmt->fetchAll(\PDO::FETCH_OBJ);
    ...
    function findAgregaData($conn, $tipo, $campoAgregacao, $tabela=null , $campo = null, $valor =null, $data1, $data2){
        $tabela = ($tabela) ? $tabela: $this->tabela;
        try {
            if($campo!=null && $valor!=null){
                $condicao = " WHERE " . $campo . " =:campo AND (data_abertura BETWEEN '$data1' AND '$data2 23:59:59')";
            }else{
                $condicao = "";
            }
    ...
    function findExeto($conn, $tipo, $campoAgregacao, $tabela=null , $campo = null, $valor =null  ){
        $tabela = ($tabela) ? $tabela: $this->tabela;
        try {
            if($campo!=null && $valor!=null){
                $condicao = " WHERE " . $campo . " !=:campo ";
    ...
    function findData($conn, $tipo, $campoAgregacao, $tabela=null , $campo = null, $valor =null, $dataa =null  ){
        $tabela = ($tabela) ? $tabela: $this->tabela;
        try {
            if($campo!=null && $valor!=null){
                $datahoje = date('Y-m-d');             
                $condicao = " WHERE " . $campo . " !=:campo AND ".$dataa." > '".$datahoje."'";
            }else{
                $condicao = "";
            }
    ...
    function findExetoData($conn, $tipo, $campoAgregacao, $tabela=null , $campo = null, $valor =null, $dataf =null  ){
        $tabela = ($tabela) ? $tabela: $this->tabela;
        try {
            if($campo!=null && $valor!=null){
                $datahoje = date('Y-m-d');
                $condicao = " WHERE " . $campo . " =:campo AND ".$dataf." = '".$datahoje."'";
            }else{
                $condicao = "";
            }
